...
class ActionCfg:
    ...
    leg_pos = mdp.actions.HipMimicPositionActionCfg(
        ...
        scale=0.5,   # HẠ từ 1.0 → ±0.5 rad ≈ ±28°: chặn robot gập chân sâu tới mức knee/hip quét đất
...
class CommandsCfg:
    ...
    velocity_command = commands.UniformVelocityCommandCfg(
        ...
        ranges=commands.UniformVelocityCommandCfg.Ranges(
            ...
            lin_vel_y=(-0.4, 0.4),      # CURRICULUM: hạ từ ±1.0 — học balance + đi thẳng tốc thấp trước
            ang_vel_z=(0.0, 0.0),       # CURRICULUM: TẮT xoay — cô lập balance fore-aft (nới lại sau khi vững)
            heading=(0.0, 0.0),
        ),
    )
