"""ArticulationCfg cho robot V5 — load trực tiếp từ USD (Onshape export).
...
LEGGED_ROBOT_V5_USD_PATH = os.path.join(
    CURRENT_DIR, "usd_file", "robot_v5_fixed.usd",  # bánh xe đã gỡ khỏi vòng kín (fix_loop_v5.py)
)


LEGGED_V5_CFG = ArticulationCfg(
    spawn=UsdFileCfg(
        usd_path=LEGGED_ROBOT_V5_USD_PATH,
    ...
    actuators={
        ...
        "knee_passive": IdealPDActuatorCfg(
            joint_names_expr=[
                "right_knee_joint_1", "right_knee_joint_2",
        ...
        # Bánh xe: IdealPD (explicit) — tính torque = damping*(v_target - v) trong
        # Python rồi set effort. PhysX velocity-drive (Implicit) KHÔNG sinh mô-men
        # trên joint này; còn effort thuần thì quay tới velocity_limit ngay (đã verify).
