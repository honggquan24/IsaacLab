...
Usage:
    ./isaaclab.sh -p source/isaaclab_assets/isaaclab_assets/evobot_v1/test/pid_tests/test_pid_balance_simple.py \
...
def save_data(data_log, output_dir, timestamp, params):
    ...
    with open(csv_file, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['step', 'time', 'pitch', 'error', 'pid_output',
                        'p_term', 'i_term', 'd_term', 'left_wheel', 'right_wheel', 'height'])
    ...
    fig.suptitle(f'PID Balance - Pitch Control\nKP={params["kp"]}, KI={params["ki"]}, KD={params["kd"]}',
...
def main():
    ...
    pids = [SimplePIDController(args.kp, args.ki, args.kd, args.target_pitch)
        ...
        for step in range(args.max_steps):
            ...
            for i in range(args.num_envs):
                ...
                if i == 0:  # Log first env
                    ...
                    log_data = [step, step * dt, pitch[0].item(), error, output,
                               p, i_term, d, output, output, height]
