...
Usage:
    ...
    ./isaaclab.sh -p source/isaaclab_assets/isaaclab_assets/evobot_v1/test/rl_tests/test_rl_pitch_tracking.py \
...
parser.add_argument("--command_change_threshold", type=float, default=0.05, help="Threshold to detect command change (radians)")
...
def main():
            ...
            with torch.inference_mode():
                ...
                if elapsed_time >= (current_step_idx + 1) * args_cli.step_duration:
                    ...
                    if current_step_idx < len(step_values):
                        ...
                        print(f"\n[STEP {current_step_idx}] New target: {step_values[current_step_idx]:.3f} rad ({np.rad2deg(step_values[current_step_idx]):.2f}°)")
                        print(f"{'Time':>8s} {col_name:>10s} {col_actual:>10s} {'Error':>10s} {'Height':>10s}")
                        print("-" * 68)
                ...
                if step_count % args_cli.log_interval == 0:
                    ...
                    if step_count % 50 == 0:
                        ...
                        print(
                            f"{elapsed_time:8.2f} {np.rad2deg(angle_cmd_value):10.2f} {np.rad2deg(angle_actual_value):10.2f} {np.rad2deg(angle_error):10.2f} {height_value:10.3f}"
                        )
    ...
    for i in range(len(step_times) - 1):
        metrics = compute_step_metrics(time_array, angle_cmd_array, angle_actual_array, step_times[i], step_times[i + 1])
        if metrics:
            ...
            print(f"    Steady-state error: {metrics['steady_state_error']:+.4f} rad ({np.rad2deg(metrics['steady_state_error']):+.3f}°)")
