...
class ObservationsCfg:
    ...
    class PolicyCfg(ObservationGroupCfg):
        ...
        # arm_position_cmd = ObservationTermCfg(
        #     func=observations.generated_commands,
        #     params={"command_name": "arm_position"},
        # )
        # gripper_cmd = ObservationTermCfg(
        #     func=observations.generated_commands,
        #     params={"command_name": "gripper_command"},
        # )
...
@configclass
class CommandsCfg:
    """Command configuration for multi-task control.

    This configuration enables the robot to:
    1. Navigate using differential drive (velocity commands)
    2. Control arm height (position commands)
    3. Control gripper open/close (binary commands)

    Command dimensions:
    - base_velocity: 3D (lin_vel_x, lin_vel_y, ang_vel_z)
    - arm_position: 1D (target joint position in radians)
    - gripper_command: 1D (0=open, 1=close)
    Total: 5D command space
    """

    # (1) Base velocity command for navigation
    base_velocity = commands.UniformVelocityCommandCfg(
        asset_name="robot",
        resampling_time_range=(1.0, 2.0),  # Resample every 5-10 seconds
        rel_standing_envs=0.1,  # 10% of time standing still (vx=vy=omega=0)
        rel_heading_envs=0.0,   # 0% heading-only (we want full 3D velocity)
        heading_command=False,  # Use angular velocity (not heading angle)
        debug_vis=True,
        ranges=commands.UniformVelocityCommandCfg.Ranges(
            # Forward/backward velocity (m/s)
            lin_vel_x=(-0.5, 0.5),
            # Lateral velocity (m/s) - set to 0 for differential drive
            lin_vel_y=(0.0, 0.0),
            # Angular velocity (rad/s) - turning
            ang_vel_z=(-1.0, 1.0),
            # Heading angle (not used since heading_command=False)
            heading=(0.0, 0.0),
        ),
    )

    # # (2) Arm position command for manipulation
    # arm_position = mdp.commands.UniformJointPositionCommandCfg(
    #     asset_name="robot",
    #     joint_name="arm_joint",
    #     resampling_time_range=(1.0, 2.0),  # Resample arm target every 3-7s
    #     ranges=mdp.commands.UniformJointPositionCommandCfg.Ranges(
    #         joint_pos=(-1.57, 1.57),  # -90° to +90° (adjust based on robot limits)
    #     ),
    # )

    # # (3) Gripper command for grasping
    # gripper_command = mdp.commands.BinaryGripperCommandCfg(
    #     asset_name="robot",
    #     resampling_time_range=(1.0, 2.0),  # Resample gripper state every 2-5s
    #     open_prob=0.5,  # 50% open, 50% close (balanced)
    # )
...
class EvobotV1EnvCfgBalance(ManagerBasedRLEnvCfg):
    ...
    commands: CommandsCfg = CommandsCfg()  # NEW: Multi-task commands
