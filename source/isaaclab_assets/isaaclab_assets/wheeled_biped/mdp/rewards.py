...
"""Reward term riêng của robot bipedal wheel (V5)."""
...
from isaaclab.utils.math import euler_xyz_from_quat, quat_apply_inverse, quat_mul, wrap_to_pi, yaw_quat
...
def velocity_settling_bonus(
    ...
    return ((torch.abs(vy_err) < band_vel) & (torch.abs(yr_err) < band_yaw)).float()
...
def hip_symmetry_l2(
    env: ManagerBasedRLEnv,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot", joint_names=["right_hip_joint", "left_hip_joint"]),
) -> torch.Tensor:
...
def track_base_height_above_axle_exp(
    ...
    z_axle = (asset.data.body_pos_w[:, right_idx, 2] + asset.data.body_pos_w[:, left_idx, 2]) * 0.5
...
def wheel_speed_symmetry_l2(
    ...
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot", joint_names=["left_wheel_joint", "right_wheel_joint"]),
) -> torch.Tensor:
