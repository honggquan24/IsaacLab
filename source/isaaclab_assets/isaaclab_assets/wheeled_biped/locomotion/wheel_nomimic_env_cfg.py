...
"""Wheeled locomotion environment cho robot bipedal wheel (V5) — clone từ V0, đổi robot + joint names.
...
@configclass
class WheeledBipedSceneCfg(InteractiveSceneCfg):
    num_envs: int = 1
    ...
    robot: Articulation = WHEELED_BIPED_CFG.replace(
...
@configclass
class ActionCfg:
    # KHÔNG mimic: policy điều khiển trực tiếp cả 4 khớp hip (2 active + 2 mimic).
    leg_pos = actions.JointPositionActionCfg(
        asset_name="robot",
        joint_names=[
            "right_hip_joint",
            "right_hip_joint_mimic",
            "left_hip_joint",
            "left_hip_joint_mimic",
        ],
        scale=1.0,  # action [-1,1] -> ±1.0 rad ≈ ±57°, khớp limit hip ±60°
    )
...
class CommandsCfg:
    velocity_command = commands.UniformVelocityCommandCfg(
        ...
        rel_standing_envs=0.4,  # giảm từ 0.7 — cho robot tập DI CHUYỂN nhiều hơn (đang chỉ học đứng)
...
class RewardCfg:
    ...
    track_lin_vel_xy_exp = RewardTermCfg(
        ...
        weight=1.5,  # tăng từ 1.0 — nhấn mạnh việc chạy theo lệnh
        params={"command_name": "velocity_command", "std": 1.0},  # std 0.5→1.0: gradient học chạy từ trạng thái đứng
    )

    track_ang_vel_z_exp = RewardTermCfg(
        func=mdp_vel.track_ang_vel_z_world_exp,
        weight=1.5,
        params={"command_name": "velocity_command", "std": 1.0},
    )
    ...
    leg_joint_acc = RewardTermCfg(
        ...
        weight=-5.0e-7,  # bù cho việc giữ hip damping thấp (1.0) — khử rung qua reward, không qua mechanical damping
        params={
...
@configclass
class WheeledBipedWheelNoMimicEnvCfg(ManagerBasedRLEnvCfg):
    scene: WheeledBipedSceneCfg = WheeledBipedSceneCfg(num_envs=1, env_spacing=2.0)
