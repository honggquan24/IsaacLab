...
def goal_reached(
    env: ManagerBasedRLEnv, command_name: str, asset_cfg: SceneEntityCfg, threshold: float = 0.3
) -> torch.Tensor:
...
def goal_distance_exp(
    env: ManagerBasedRLEnv, command_name: str, asset_cfg: SceneEntityCfg, std: float = 1.0
) -> torch.Tensor:
    """Thưởng Gaussian theo khoảng cách tới đích (đỉnh tại đích)."""
    return torch.exp(-_goal_dist_b(env, command_name) / std)


def heading_toward_goal(
    env: ManagerBasedRLEnv, command_name: str, asset_cfg: SceneEntityCfg, v_ref: float = 0.3
) -> torch.Tensor:
