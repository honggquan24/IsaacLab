"""Biped-Outer-Vel-Direct — Outer loop: velocity command → tilt setpoint trực tiếp → inner.

Kiến trúc:
    vel_cmd (vx, vy, yaw_rate)
        └─ Outer RL (3 dim) ──► (roll_des, pitch_des, yaw_rate_raw)
                                         └─ Inner TiltPIDAction (fixed gains)
                                                └─ torque

RL outputs 3 values trực tiếp: [roll_raw, pitch_raw, yaw_rate_raw] ∈ [-1, 1]
  roll_des      = roll_raw   × 0.30 rad  (lean forward/back)
  pitch_des     = pitch_raw  × 0.20 rad
  yaw_des       += yaw_rate_raw × 1.57 rad/s × dt  (integrate)

Không có velocity PID — RL học mapping vel_error → tilt_cmd end-to-end.
Inner TiltPIDAction chạy với reference gains (không RL-tune).

Obs (23-dim):
    velocity_command(3) + velocity_error(3) + base_lin_vel_b(3)
    + ang_vel_b(3) + projected_gravity(3) + wheel_vel(2) + last_action(3) = 20-dim

Train:
    ./isaaclab.sh -p scripts/reinforcement_learning/rsl_rl/train.py \\
        --task Biped-Outer-Vel-Direct --num_envs 1024 --headless

Play:
    ./isaaclab.sh -p scripts/reinforcement_learning/rsl_rl/play.py \\
        --task Biped-Outer-Vel-Direct --num_envs 4
"""
...
@configclass
class OuterVelDirectSceneCfg(InteractiveSceneCfg):
...
@configclass
class ActionCfg:
    """3 outer outputs (roll_des, pitch_des, yaw_rate) + embedded inner TiltPIDAction."""

    outer_direct = mdp.OuterVelDirectActionCfg(
...
class ObservationsCfg:
    """Obs 20-dim:
    velocity_command(3) + velocity_error(3) + base_lin_vel_b(3)
    + ang_vel_b(3) + projected_gravity(3) + wheel_vel(2) + last_action(3)
    """
...
@configclass
class BipedOuterVelDirectEnvCfg(ManagerBasedRLEnvCfg):
    """Outer loop: velocity command → tilt setpoint trực tiếp → inner TiltPID.

    Sim 200 Hz. Policy (outer) 50 Hz (decimation=4). Episode 20 s.
    """

    scene:        OuterVelDirectSceneCfg = OuterVelDirectSceneCfg(num_envs=512, env_spacing=2.5)
