...
class NavCommandsCfg:
    ...
    height_command = commands.UniformPoseCommandCfg(
        ...
        ranges=commands.UniformPoseCommandCfg.Ranges(
            pos_x=(0.0, 0.0), pos_y=(0.0, 0.0), pos_z=(0.30, 0.30),  # = 0.30 khớp ĐÚNG lúc train low-level
...
class NavEventCfg:
    ...
    reset_position = EventTermCfg(
        ...
        params={
            "asset_cfg": SceneEntityCfg(name="robot"),
            "pose_range": {"x": (-0.5, 0.5), "y": (-0.5, 0.5)},   # CURRICULUM STAGE-1: spawn gần gốc để robot→đích ≈1m (khớp dải đích ±1). BỎ yaw: base xoay 90°X nên yaw làm robot LẬT. Khôi phục: (-3,3)
...
class NavRewardCfg:
    ...
    termination_penalty = RewardTermCfg(func=rewards.is_terminated, weight=-10.0)   # hạ từ -200: -200 tạo spike phương sai khổng lồ → value loss nổ, noise_std phình. Tầng cao không cần phạt nặng vụ ngã (việc của low-level).
