...
def main():
    ...
    robot = Articulation(LEGGED_V5_CFG.replace(prim_path="/World/Robot"))
    sim.reset()
    ...
    for step in range(60):
        ...
        if step % 15 == 14:
            ...
            print(f"[step {step+1:3d}] joint_vel(R,L)=[{wjv[0]:+.2f},{wjv[1]:+.2f}]  "
                  f"wheelR_angvel_w={[f'{v:+.2f}' for v in waw[0].tolist()]}  "
                  f"wheelL_angvel_w={[f'{v:+.2f}' for v in waw[1].tolist()]}")
            print(f"            root_lin_vel_w={[f'{v:+.3f}' for v in rlv]}  "
                  f"root_ang_vel_w={[f'{v:+.3f}' for v in rav]}")
