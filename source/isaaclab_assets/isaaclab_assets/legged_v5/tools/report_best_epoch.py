...
for c in ckpts:
    ...
    score = 0.40 * norm(r, r_lo, r_hi) + 0.30 * norm(e, e_lo, e_hi) \
            + 0.15 * t + 0.15 * (1 - b)
...
for c, r, e, t, b, s in rows_sorted[:5]:
    print(f"  iter {c:4d}  | score {s:.3f} | reward~{r:6.1f} | eplen~{e:5.0f}"
...
print(f"    (đỉnh reward THÔ tại iter {steps[ipk]}: reward={reward[ipk]:.1f}, "
