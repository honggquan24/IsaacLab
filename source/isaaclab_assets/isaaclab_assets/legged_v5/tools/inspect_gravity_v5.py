...
from isaaclab_assets.legged_v5.locomotion.legged_v5_wheel_env_cfg import LeggedV5WheelEnvCfg

import os
_OUT = open(os.path.join(os.path.dirname(os.path.dirname(__file__)), "probe_gravity_log.txt"), "w")
_bp = print
def print(*a, **k):  # noqa
    s = " ".join(str(x) for x in a); _bp(s, flush=True); _OUT.write(s + "\n"); _OUT.flush()

cfg = LeggedV5WheelEnvCfg(); cfg.scene.num_envs = 1; cfg.sim.device = "cuda:0"
env = ManagerBasedRLEnv(cfg=cfg); env.reset()
robot = env.scene["robot"]; DEV = env.device
...
def set_and_read(label, dq_world):
    ...
    env.sim.step(render=False); robot.update(cfg.sim.dt)
