...
\begin{table}[H]
    ...
    \hline
    0 & \texttt{right\_hip\_joint} & 0{,}5 & Vị trí $\to$ IdealPD ($K_p{=}30$, $K_d{=}0{,}5$) \\
    1 & \texttt{left\_hip\_joint}  & 0{,}5 & Vị trí $\to$ IdealPD ($K_p{=}30$, $K_d{=}0{,}5$) \\
    2 & \texttt{right\_wheel\_joint} & 10{,}0 & Vận tốc $\to$ Implicit ($K_p{=}0$, $K_d{=}50$) \\
    3 & \texttt{left\_wheel\_joint}  & 10{,}0 & Vận tốc $\to$ Implicit ($K_p{=}0$, $K_d{=}50$) \\
    \hline
    \end{tabular}
\end{table}
...
\begin{equation}
    ...
    q_{B,des} = -\,q_{A,des},\qquad s_{hip}=0{,}5
    \label{eq:mimic_apply}
\end{equation}
Hệ số $s_{hip}=0{,}5$ (giới hạn lệnh góc hông trong $\pm0{,}5$~rad~$\approx
\pm28^\circ$) được hạ từ $1{,}0$ để chặn robot gập chân quá sâu khiến khớp
hông/gối quét đất (\texttt{illegal\_contact}); dải này vẫn đủ rộng để chỉnh
chiều cao thân trong khoảng $0{,}25$--$0{,}35$~m. Khi reset, khớp mimic cũng
được ghi $q_B = -q_A$ ngay trước bước đầu để vòng kín 5-bar luôn thoả ràng buộc.
...
được áp lên thành phần $v_{y,des}$, lấy mẫu trong khoảng $[-0{,}5,\;0{,}5]$~m/s.
...
khoảng $[-0{,}5,\;0{,}5]$~rad/s, cho phép robot học các thao tác quay trái,
...
\begin{table}[H]
    ...
    & $v_{y,des}$ & $[-0{,}5,\;+0{,}5]$ m/s & Tiến/lùi (trục dọc thân) \\
    & $\omega_{z,des}$ & $[-0{,}5,\;+0{,}5]$ rad/s & Quay tại chỗ \\
    \hline
    \texttt{height\_command} & $z_{des}$ & $0{,}30$ m (cố định) & Độ cao thân tương đối so với trục bánh \\
    \hline
    \end{tabular}
\end{table}
...
phương trình~\eqref{eq:rew_height}), giữ cố định $0{,}30$~m và resample mỗi
...
Hàm phần thưởng tổng hợp tại mỗi bước $t$ là tổng có trọng số của 13 số hạng
...
\textbf{(2) Bám vận tốc dọc trục $x$} (\texttt{track\_lin\_vel\_x\_exp},
$w=+0{,}5$): Gaussian kernel trên sai lệch vận tốc \emph{ngang} thân trong hệ
yaw-frame, $\sigma=1{,}0$~m/s. Do $v_{x,des}$ bị khoá bằng 0, số hạng này
chủ yếu \emph{chống trôi ngang} (trọng số nhỏ):
\begin{equation}
    r_{lin,x} = \exp\!\left(-\frac{e_{vx}^2}{\sigma^2}\right),
    \quad e_{vx}=v_{x,des}-v_x,\quad \sigma=1{,}0
    \label{eq:rew_lin_x}
\end{equation}

\textbf{(3) Bám vận tốc dọc trục $y$} (\texttt{track\_lin\_vel\_y\_exp},
$w=+1{,}5$): Gaussian kernel trên sai lệch vận tốc \emph{tiến} (trục dọc thân
là $y$) trong hệ yaw-frame, $\sigma=1{,}0$~m/s — tín hiệu bám tốc chính. Việc
tách $xy\to x,y$ riêng giúp theo dõi từng trục trên log khi gỡ lỗi:
\begin{equation}
    r_{lin,y} = \exp\!\left(-\frac{e_{vy}^2}{\sigma^2}\right),
    \quad e_{vy}=v_{y,des}-v_y,\quad \sigma=1{,}0
    \label{eq:rew_lin_y}
\end{equation}

\textbf{(4) Bám vận tốc góc} (\texttt{track\_ang\_vel\_z\_exp}, $w=+1{,}5$):
...
dọc} (fore-aft lean) để tăng/giảm tốc; vì vậy đề tài phạt nghiêng ngang
\textbf{nặng} ($w=-50$) nhưng chỉ phạt nghiêng dọc \textbf{rất nhẹ} ($w=-5$)
--- đủ để chống lật về phía trước/sau mà vẫn cho phép robot lean khi tăng tốc,
thay cho cặp \texttt{upright}/\texttt{rpy} cứng của robot chân.
...
\textbf{(8) Phạt nghiêng dọc} (\texttt{fore\_aft\_tilt}, $w=-5$): cùng dạng
bình phương thành phần trọng lực nhưng theo trục dọc thân (trục body-$z$ sau
khi xoay $90^\circ$). Trọng số nhỏ ($-5$ so với $-50$ của nghiêng ngang) để
chống lật trước/sau mà vẫn cho phép robot lean khi tăng/giảm tốc:
\begin{equation}
    r_{fa} = g_{b,z}^2
    \label{eq:rew_foreaft}
\end{equation}

\textbf{(9) Căn tâm khối trên mặt phẳng bánh} (\texttt{com\_wheel\_plane},
...
\textbf{(10) Đối xứng hai chân} (\texttt{hip\_symmetry}, $w=-10$): do mỗi chân
...
\textbf{(11) Đứng yên khi không có lệnh} (\texttt{stand\_still}, $w=-2{,}0$):
...
\textbf{(12) Tốc độ thay đổi hành động} (\texttt{action\_rate}, $w=-0{,}07$):
...
\textbf{(13) Gia tốc khớp hông} (\texttt{leg\_joint\_acc},
...
\begin{table}[H]
    ...
    \texttt{illegal\_contact\_hip}  & lực chạm khớp hông $> 200$~N          & $-200$ \\
    \texttt{illegal\_contact\_knee} & lực chạm khớp gối $> 200$~N           & $-200$ \\
    \hline
    \end{tabular}
\end{table}
...
\emph{tiến} của robot và hướng tới đích. Lệnh đích $\mathbf{g}_b$ do
\texttt{UniformPose2dCommand} trả về \textbf{đã ở body yaw-frame} (vector
robot$\to$đích), và do thân xoay $90^\circ$ quanh $X$ nên \textbf{trục tiến của
robot là $+Y$ của yaw-frame} (index 1, khớp quy ước \texttt{track\_lin\_vel\_y}).
Vì vậy phần thưởng là thành phần $Y$ của hướng đích đơn vị:
\begin{equation}
    r_{head} = \operatorname{clamp}\!\Bigl(
        \frac{g_{b,y}}{\lVert\mathbf{g}_{b,xy}\rVert},
        \,-1,\,1\Bigr),\qquad
    \mathbf{g}_b = \text{vị trí đích trong body yaw-frame}
...
\begin{table}[H]
    ...
    \hline
    Kiến trúc Actor & $[256,256]$ (ELU) &
    Hai tầng ẩn $256{\to}256$, kích hoạt ELU,
    $\sigma_{init}{=}1{,}0$. \\
    \hline
    Kiến trúc Critic & $[256,256]$ (ELU) &
...
\subsubsection{Mở rộng: điều hướng tránh vật cản bằng LiDAR}
\label{sec:nav_lidar}

Phiên bản điều hướng cơ bản giả định không gian trống. Để robot di chuyển an
toàn trong môi trường có vật cản, đề tài xây dựng biến thể
\texttt{Isaac-Legged-V5-Warehouse-Nav}: thay mặt phẳng bằng mô hình kho
\textit{Simple\_Warehouse} (nạp làm terrain USD dùng chung), trang bị thêm một
cảm biến \textbf{LiDAR 2D} và bổ sung tín hiệu tránh vật cản vào quan sát,
phần thưởng và điều kiện kết thúc của vòng ngoài.

\textbf{Cảm biến LiDAR.} LiDAR được dựng bằng \texttt{RayCasterCfg} kết hợp
\texttt{LidarPatternCfg}, gắn trên thân (\texttt{base}) tại độ cao
$0{,}3$~m so với gốc thân, raycast vào mesh kho. Cấu hình tia tóm tắt trong
Bảng~\ref{tab:lidar_cfg}.

\begin{table}[H]
    \centering
    \caption{Cấu hình cảm biến LiDAR 2D}
    \label{tab:lidar_cfg}
    \renewcommand{\arraystretch}{1.3}
    \begin{tabular}{|l|c|l|}
    \hline
    \textbf{Tham số} & \textbf{Giá trị} & \textbf{Mô tả} \\
    \hline
    Trường quét ngang & $[-180^\circ,\,+180^\circ]$ & Quét trọn $360^\circ$ \\
    Độ phân giải ngang & $4^\circ$ & Bước góc giữa hai tia \\
    Số tia & 90 & $= 360^\circ / 4^\circ$ \\
    Số kênh dọc & 1 & LiDAR 2D (mặt phẳng ngang) \\
    Tầm xa tối đa & $10{,}0$~m & Tia không trúng $\to$ kẹp về $10$~m \\
    Căn chỉnh tia & \texttt{yaw} & Tia nằm ngang, xoay theo hướng thân \\
    Vị trí đặt & $(0,0,0{,}3)$~m trên \texttt{base} & Offset so với gốc thân \\
    \hline
    \end{tabular}
\end{table}

\textbf{Quan sát bổ sung.} Vector $90$ tia khoảng cách được chuẩn hoá về
$[0,1]$ (chia cho tầm xa tối đa; $1$ = không thấy vật cản) rồi nối thêm vào
quan sát của Actor vòng ngoài, nâng số chiều từ $16$ lên $16+90 = 106$:
\begin{equation}
    \texttt{lidar\_scan}_i = \frac{\min(d_i,\,d_{max})}{d_{max}}\in[0,1],
    \qquad d_{max}=10{,}0~\text{m}
    \label{eq:lidar_scan}
\end{equation}
trong đó $d_i$ là khoảng cách từ gốc cảm biến tới điểm va chạm của tia thứ $i$.

\textbf{Phần thưởng tránh vật cản} (\texttt{obstacle\_avoidance}, $w=-2{,}0$):
phạt khi tia gần nhất $d_{min}$ lọt vào vùng an toàn $d_{safe}=0{,}6$~m, tăng
tuyến tính từ $0$ (xa) tới $1$ (sát vật cản):
\begin{equation}
    r_{obs} = \frac{\operatorname{clamp}(d_{safe}-d_{min},\,0,\,d_{safe})}{d_{safe}},
    \qquad d_{safe}=0{,}6~\text{m}
    \label{eq:rew_obstacle}
\end{equation}

\textbf{Điều kiện kết thúc va chạm} (\texttt{collision}): episode kết thúc khi
tia gần nhất $d_{min} < 0{,}25$~m (coi như sắp đâm vật cản); đây là lớp an toàn
bổ sung bên cạnh va chạm vật lý thật với collider của kho.

\textbf{Đích và sinh lại.} Đích điều hướng được giới hạn trong lòng kho
$x_g\in[-5,5]$~m, $y_g\in[-3,3]$~m; robot sinh lại quanh tâm kho
$x,y\in[-1,1]$~m với hướng ngẫu nhiên $\psi\in[-\pi,\pi]$. Do mesh kho dùng
chung toạ độ thế giới, cấu hình này chạy với \texttt{num\_envs}${=}1$ cho
demo/suy luận; huấn luyện song song cần \texttt{MultiMeshRayCaster} để mỗi env
thấy đầy đủ hình học vật cản.
...
\texttt{IdealPD} không sinh mô-men trên joint velocity-drive của bánh nên buộc
dùng Implicit):
...
\begin{table}[H]
    ...
    \hline
    Hông chủ động (R, L)   & Vị trí (IdealPD)   & 30{,}0  & 0{,}5 & 100   & Chính sách điều khiển \\
    Hông mimic (R, L)      & Vị trí (IdealPD)   & 30{,}0  & 0{,}5 & 100   & Setpoint $=-$hông chủ động \\
    Gối bị động ($\times4$) & Tự do (IdealPD)    & 0{,}0   & 0{,}0 & 100   & Coupler vòng kín \\
    Bánh xe (R, L)         & Vận tốc (Implicit) & 0{,}0   & 50{,}0& 500   & Chính sách điều khiển \\
...
\begin{itemize}
    ...
    \begin{equation}
        q_{A,des} = a_{hip}\times 0{,}5,\qquad q_{B,des} = -\,q_{A,des}
        \label{eq:hip_setpoint}
    \end{equation}
    IdealPD tính mô-men theo luật PD với $K_p = 30$, $K_d = 0{,}5$ (thêm chút
    cản cơ học để dập rung khớp hip, kết hợp số hạng phạt gia tốc
    \texttt{leg\_joint\_acc} trong hàm thưởng):
    \begin{equation}
        \tau_{hip} = \operatorname{clamp}\!\bigl(30\,(q_{des}-q) - 0{,}5\,\dot{q},\;\pm100\bigr)\;\mathrm{[N{\cdot}m]}
    ...
    \end{equation}
    Bộ truyền động ẩn hoạt động ở chế độ cản thuần ($K_p=0$, $K_d=50$), PhysX
    giải mô-men $\tau_{wheel}=K_d(\omega_{des}-\omega)$ và bão hoà ở
    $\pm500$~N$\cdot$m:
    \begin{equation}
        \tau_{wheel} = \operatorname{clamp}\!\bigl(50\,(\omega_{des}-\omega),\;\pm500\bigr)\;\mathrm{[N{\cdot}m]}
...
\node[mathblock] (pd_hip) at (-2.7, -4.0)
    {\textbf{IdealPD — Hông}\\[2pt]
     $K_p{=}30,\;K_d{=}0{,}5,\;\tau_{max}{=}{\pm}100\;\text{N{\cdot}m}$\\[2pt]
     $\tau_{hip}=\mathrm{clamp}(30\Delta q-0{,}5\dot{q},\;\pm100)$};

\node[mathblock] (pd_whl) at (2.7, -4.0)
    {\textbf{Implicit — Bánh xe}\\[2pt]
     $K_p{=}0,\;K_d{=}50,\;\tau_{max}{=}{\pm}500\;\text{N{\cdot}m}$\\[2pt]
     $\tau_{whl}=\mathrm{clamp}(50\,\Delta\omega,\;\pm500)$};
...
(IdealPD, $K_p{=}30$, $K_d{=}0{,}5$, $\tau_{max}{=}{\pm}100$ N$\cdot$m, khớp
mimic lấy giá trị âm); nhánh bánh xe (ImplicitActuator, $K_p{=}0$,
$K_d{=}50$, $\tau_{max}{=}{\pm}500$ N$\cdot$m). Cả hai hội tụ tại PhysX ở 200~Hz.}
...
dùng hai lớp ẩn $[256,256]$. Bảng~\ref{tab:network_arch} liệt
...
\begin{table}[H]
    ...
    Outer Actor  & 16 & $[256\!\to\!256]$          & 3 (tuyến tính) & ELU \\
    Outer Critic & 16 & $[256\!\to\!256]$          & 1 (giá trị)    & ELU \\
    \hline
    \end{tabular}
\end{table}
