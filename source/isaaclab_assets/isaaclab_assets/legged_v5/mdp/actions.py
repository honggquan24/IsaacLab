...
class HipMimicPIDAction(ActionTerm):
    ...
    def __init__(self, cfg: HipMimicPIDActionCfg, env: ManagerBasedEnv):
        ...
        self._max_gains = torch.tensor(
            [cfg.max_kp, cfg.max_ki, cfg.max_kd], device=env.device
        ).unsqueeze(0)
    ...
    def apply_actions(self):
        ...
        r_h, kp, ki, kd, kp_lat = (
            self._gains[:, 0], self._gains[:, 1], self._gains[:, 2],
            self._gains[:, 3], self._gains[:, 4],
        )
        ...
        self._integral = torch.clamp(
            self._integral + e_h * self._dt, -cfg.integral_limit, cfg.integral_limit
        )
    ...
    def reset(self, env_ids: torch.Tensor | None = None) -> None:
        ...
        if env_ids is None:
            active_pos = self._asset.data.joint_pos[:, self._active_ids]
            self._asset.write_joint_position_to_sim(-active_pos, joint_ids=self._mimic_ids)
            self._integral.zero_(); self._prev_error.zero_()
            self._raw_actions.zero_(); self._gains.zero_(); self._processed.zero_()
        else:
            active_pos = self._asset.data.joint_pos[env_ids][:, self._active_ids]
            self._asset.write_joint_position_to_sim(-active_pos, joint_ids=self._mimic_ids, env_ids=env_ids)
            self._integral[env_ids] = 0.0; self._prev_error[env_ids] = 0.0
            self._raw_actions[env_ids] = 0.0; self._gains[env_ids] = 0.0; self._processed[env_ids] = 0.0
...
class WheelPIDBalanceAction(ActionTerm):
    ...
    def apply_actions(self):
        ...
        self._integral = torch.clamp(
            self._integral + e_bal * self._dt, -cfg.integral_limit, cfg.integral_limit
        )
