...
def rpy_alignment_imu(
    ...
    weighted_error = (w0 * torch.square(roll_error) + w1 * torch.square(pitch_error) + w2 * torch.square(yaw_error)) / (
        w0 + w1 + w2
    )
...
def height_reward(
    ...
    robot_heights = torch.where(valid_mask.any(dim=-1), sensor_pos_z - ground_z, sensor_pos_z)
...
def angular_velocity_reward(
    ...
    w0, w1, w2 = axis_weights
    weighted_vel = torch.stack([ang_vel[:, 0] * w0, ang_vel[:, 1] * w1, ang_vel[:, 2] * w2], dim=-1)
...
def linear_velocity_reward(
    ...
    w0, w1, w2 = axis_weights
    weighted_vel = torch.stack([lin_vel[:, 0] * w0, lin_vel[:, 1] * w1, lin_vel[:, 2] * w2], dim=-1)
...
def feet_contact_force_symmetry(
    ...
    contact_ok = torch.minimum(fz_l / threshold_force, fz_r / threshold_force).clamp(0.0, 1.0)
