...
from isaaclab.envs.mdp import events, terminations
...
from ..mdp import *  # noqa: F403

# Import velocity env config from correct location
from ..velocity.velocity_env_cfg import EvobotVelocityBalanceEnvCfg

# Load low-level balance environment config
LOW_LEVEL_ENV_CFG = EvobotVelocityBalanceEnvCfg()
...
class ActionsCfg:
    ...
    pre_trained_policy_action: PreTrainedBalancePolicyActionCfg = PreTrainedBalancePolicyActionCfg(
        ...
        #   --task Isaac-Evobot-Balance --num_envs 1 'agent.load_run=<run_name>' 'agent.load_checkpoint="model_.pt"'
...
class TerminationsCfg:
    ...
    left_grip_contact = TerminationTermCfg(
        func=terminations.illegal_contact,
        params={"threshold": 10.0, "sensor_cfg": SceneEntityCfg("contact_forces", body_names="gripper")},
    )

    right_grip_contact = TerminationTermCfg(
        func=terminations.illegal_contact,
        params={"threshold": 10.0, "sensor_cfg": SceneEntityCfg("contact_forces", body_names="gripper_01")},
    )
...
@configclass
class EvobotNavigationPretrainedEnvCfg(ManagerBasedRLEnvCfg):
...
@configclass
class EvobotNavigationPretrainedEnvCfgPlay(EvobotNavigationPretrainedEnvCfg):
