...
## 2. Tổng quan thứ tự — Độ khó tăng dần

| File | Tên | Số vòng PID | Paradigm | Điểm mới |
|------|-----|-------------|----------|----------|
| 01 | `altitude` | 2 vòng (1D outer + att) | Position | Đơn giản nhất: chỉ điều khiển Z |
| 02 | `position` | 2 vòng (3D outer + att) | Position | Mở rộng lên 3D (X, Y, Z) |
| 03 | `velocity` | 2 vòng (3D outer + att) | **Velocity** | Paradigm khác: điều khiển vận tốc, không phải vị trí |
| 04 | `multirate` | 2 vòng (3D outer + att) | Position | PID chạy ở tần số thấp hơn sim — thực tế phần cứng |
| 05 | `hierarchical` | **3 vòng** (pos → vel → att) | Cascade | Kiến trúc đầy đủ nhất, 3 tầng tần số khác nhau |
| 06 | `navigation` | **3 vòng** + waypoint logic | Cascade | Áp dụng vào bài toán điều hướng thực tế |

**Nguyên tắc tăng độ khó:**
1. Số chiều điều khiển tăng: Z → XYZ
2. Số vòng cascade tăng: 2 → 3 vòng
3. Paradigm khác nhau: position → velocity
4. Ràng buộc thực tế: multi-rate (tần số PID ≠ tần số sim)
5. Ứng dụng thực tế: điều hướng waypoint
