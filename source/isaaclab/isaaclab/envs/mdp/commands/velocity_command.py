...
if TYPE_CHECKING:
    ...
    from .commands_cfg import NormalVelocityCommandCfg, UniformVelocityCommandCfg, UniformVelocity2DCommandCfg
...
class UniformVelocity2DCommand(CommandTerm):
    r"""Command generator that generates a 2D velocity command parallel to XY plane from uniform distribution.

    This command generator is similar to UniformVelocityCommand, but ensures the velocity is strictly
    planar (parallel to the XY plane) without any vertical (Z-axis) component. This is useful for
    robots that should only move horizontally, such as wheeled robots or balance robots.

    The command comprises of:
    - Linear velocity in X direction (forward/backward)
    - Linear velocity in Y direction (left/right)
    - Angular velocity around Z-axis (yaw rotation)

    All velocities are given in the robot's base frame projected onto the XY plane.
    ...
    cfg: UniformVelocity2DCommandCfg
    """The configuration of the command generator."""

    def __init__(self, cfg: UniformVelocity2DCommandCfg, env: ManagerBasedEnv):
        """Initialize the command generator.
        ...
        # -- command: x vel, y vel, yaw vel (2D planar velocities only)
        self.vel_command_b = torch.zeros(self.num_envs, 3, device=self.device)
        self.heading_target = torch.zeros(self.num_envs, device=self.device)
        self.is_heading_env = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
        self.is_standing_env = torch.zeros_like(self.is_heading_env)
    ...
    @property
    def command(self) -> torch.Tensor:
        """The desired 2D planar velocity command in the base frame. Shape is (num_envs, 3).

        The command contains [vel_x, vel_y, ang_vel_z] where velocities are parallel to XY plane.
        """
        return self.vel_command_b
    ...
    def _update_metrics(self):
        ...
        max_command_time = self.cfg.resampling_time_range[1]
        max_command_step = max_command_time / self._env.step_dt
        # Project robot velocity onto XY plane for metric calculation
        robot_vel_xy = self.robot.data.root_lin_vel_b[:, :2].clone()
        # logs data
        self.metrics["error_vel_xy"] += (
            torch.norm(self.vel_command_b[:, :2] - robot_vel_xy, dim=-1) / max_command_step
        )
        self.metrics["error_vel_yaw"] += (
            torch.abs(self.vel_command_b[:, 2] - self.robot.data.root_ang_vel_b[:, 2]) / max_command_step
        )
    ...
    def _resample_command(self, env_ids: Sequence[int]):
        ...
        self.vel_command_b[env_ids, 0] = r.uniform_(*self.cfg.ranges.lin_vel_x)
        # -- linear velocity - y direction (left/right)
        self.vel_command_b[env_ids, 1] = r.uniform_(*self.cfg.ranges.lin_vel_y)
    ...
    def _update_command(self):
        ...
        Note: All velocities are kept planar (parallel to XY plane).
        """
        # Compute angular velocity from heading direction
        if self.cfg.heading_command:
            # resolve indices of heading envs
    ...
    def _debug_vis_callback(self, event):
        ...
        base_pos_w = self.robot.data.root_pos_w.clone()
        base_pos_w[:, 2] += 0.5
        # -- resolve the scales and quaternions (project to XY plane)
        vel_des_arrow_scale, vel_des_arrow_quat = self._resolve_xy_velocity_to_arrow(self.command[:, :2])
        vel_arrow_scale, vel_arrow_quat = self._resolve_xy_velocity_to_arrow(self.robot.data.root_lin_vel_b[:, :2])
        # display markers
        self.goal_vel_visualizer.visualize(base_pos_w, vel_des_arrow_quat, vel_des_arrow_scale)
    ...
    def _resolve_xy_velocity_to_arrow(self, xy_velocity: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        This ensures the arrow stays parallel to the XY plane for 2D visualization.
        """
        # obtain default scale of the marker
        ...
        arrow_scale = torch.tensor(default_scale, device=self.device).repeat(xy_velocity.shape[0], 1)
        arrow_scale[:, 0] *= torch.linalg.norm(xy_velocity, dim=1) * 3.0
        # arrow-direction (only yaw rotation, keeping arrow parallel to XY plane)
        heading_angle = torch.atan2(xy_velocity[:, 1], xy_velocity[:, 0])
        zeros = torch.zeros_like(heading_angle)
        arrow_quat = math_utils.quat_from_euler_xyz(zeros, zeros, heading_angle)
        # convert everything back from base to world frame
