...
Usage:
    ./isaaclab.sh -p ./scripts/ute/evobot/environment_tests/run_robot_rl_env.py
"""
...
parser = argparse.ArgumentParser(description="Debug Evobot V1 sensors")
...
def main():
    ...
    _command = env.command_manager.get_command("grip_ee_pose_left")
    ...
    while simulation_app.is_running():
        with torch.inference_mode():
            ...
            _pitch = torch.asin(2 * (quat[:, 0] * quat[:, 2] - quat[:, 3] * quat[:, 1]))
            _yaw = torch.atan2(
                2 * (quat[:, 0] * quat[:, 3] + quat[:, 1] * quat[:, 2]), 1 - 2 * (quat[:, 2] ** 2 + quat[:, 3] ** 2)
            )

            # 3. Root height
            _height = robot.data.root_pos_w[:, 2]

            # 4. Angle from bad_orientation calculation
            _angle = torch.acos(-proj_grav[:, 2]).abs()
