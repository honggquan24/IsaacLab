...
Usage:
    ...
    ./isaaclab.sh -p scripts/ute/evobot/pid/test_pid_autotune.py
...
parser.add_argument(
    "--step_shrink", type=float, default=0.95, help="Step size decrease factor (closer to 1 = faster convergence)"
)

# Evaluation parameters
parser.add_argument(
    "--eval_steps", type=int, default=600, help="Steps per evaluation (10 seconds at 60Hz for better stability)"
)
parser.add_argument(
    "--cost_weight_action", type=float, default=0.01, help="Weight for action cost (lower = focus more on error)"
)
    ...
    def tune(self) -> tuple[list[float], float]:
        ...
        while sum(self.steps) > self.tolerance and iteration < self.max_iterations:
            ...
            for i in range(len(self.gains)):
                ...
                print(
                    f"  Param {i} ({original_gain:.4f}): +{self.steps[i]:.4f} → {test_gains[i]:.4f}, cost={cost_increase:.6f}",  # noqa: E501
                    end="",
                )
        ...
        print(
            f"  Kp_pitch={self.best_gains[3]:.4f}, Ki_pitch={self.best_gains[4]:.4f}, Kd_pitch={self.best_gains[5]:.4f}"
        )
        print("=" * 80 + "\n")
...
def main():
    ...
    print("=" * 80)
    print("./isaaclab.sh -p scripts/ute/evobot/pid/test_pid_balance_response.py \\")
