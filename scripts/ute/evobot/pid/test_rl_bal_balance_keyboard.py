...
Usage:
    ...
    ./isaaclab.sh -p scripts/ute/evobot/pid/test_rl_bal_balance_keyboard.py \
...
# Control frequency
parser.add_argument(
    "--pid_decimation", type=int, default=4, help="PID decimation factor (PID freq = RL freq / decimation)"
)
...
class BalanceKeyboardController:
    ...
    def update(self):
        ...
        if self._is_key_pressed(carb.input.KeyboardInput.DOWN) or self._is_key_pressed(
            carb.input.KeyboardInput.NUMPAD_2
        ):
        ...
        if self._is_key_pressed(carb.input.KeyboardInput.LEFT) or self._is_key_pressed(
            carb.input.KeyboardInput.NUMPAD_4
        ):
        ...
        if self._is_key_pressed(carb.input.KeyboardInput.RIGHT) or self._is_key_pressed(
            carb.input.KeyboardInput.NUMPAD_6
        ):
...
class BalancePIDController:
    ...
    def compute(
        ...
        u_roll = (
            self.kp_roll * error_roll + self.ki_roll * self.error_integral_roll + self.kd_roll * error_derivative_roll
        )
