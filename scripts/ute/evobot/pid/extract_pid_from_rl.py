        ...
        for traj_idx in range(num_trajectories):
            ...
            if randomize_initial and traj_idx > 0:
                ...
                with torch.no_grad():
                    ...
                    _random_roll = torch.rand(self.env.unwrapped.num_envs, device=self.device) * 0.3 - 0.15
                    _random_pitch = torch.rand(self.env.unwrapped.num_envs, device=self.device) * 0.3 - 0.15
...
class PIDExtractor:
    ...
    def extract(self, data: dict[str, np.ndarray]) -> dict[str, float]:
        ...
        print(
            f"  Roll:  Kp={self.gains['kp_roll']:.4f}, Ki={self.gains['ki_roll']:.4f}, Kd={self.gains['kd_roll']:.4f}"
        )
        print(
            f"  Pitch: Kp={self.gains['kp_pitch']:.4f}, Ki={self.gains['ki_pitch']:.4f}, Kd={self.gains['kd_pitch']:.4f}"  # noqa: E501
        )
...
def main():
    ...
    from isaaclab_assets.evobot.navigation.velocity.agents.rsl_rl_ppo_cfg import EvobotVelocityPPORunnerCfg
    ...
    actor_critic = ActorCritic(obs_dict, obs_groups, num_actions, **agent_cfg.policy.to_dict()).to(args_cli.device)
    ...
    print(
        f"    --kp_pitch {gains['kp_pitch']:.4f} --ki_pitch {gains['ki_pitch']:.4f} --kd_pitch {gains['kd_pitch']:.4f}"
    )
    print("=" * 80)
