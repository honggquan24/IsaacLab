...
Usage:
    ./isaaclab.sh -p scripts/ute/evobot/pid/test_pid_balance_paper.py \
...
class ComplementaryFilter:
    ...
    def update(self, gyro_rate, acc_angle):
        ...
        self.filtered_angle = self.alpha * (self.filtered_angle + gyro_rate * self.dt) + (1 - self.alpha) * acc_angle
...
def plot_response(data_log, filename, params):
    ...
    fig.suptitle(
        "PID Balance Controller Response\n"
    ...
    ax1.axhline(
        y=np.rad2deg(params["setpoint"]),
        color="r",
        linestyle="--",
        linewidth=2,
        label=f"Setpoint ({params['setpoint']}°)",
    )
...
def main():
    ...
    print("=" * 70)
    print("Paper: 'A Two Wheel Self-Balancing Vehicle' (2021)")
    print("Authors: Ďuriš et al., Slovak University of Technology")
    print("=" * 70)
    ...
    pids = [BalancePIDController(kp=args.kp, ki=args.ki, kd=args.kd, setpoint=args.setpoint) for _ in range(num_envs)]
    ...
    print(f"{'Step':>5} | {'Time':>6} | {'Gyro':>8} | {'Acc':>8} | {'Filter':>8} | {'PID':>8} | {'Height':>7}")
    print("-" * 70)
    ...
    try:
        for step in range(args.max_steps):
            ...
            for i in range(num_envs):
                filtered_angles[i] = filters[i].update(gyro_rate_y[i].item(), acc_angle[i].item())

            # 4. Compute PID output
            wheel_efforts = torch.zeros(num_envs, device=env.device)
            pid_outputs = []
            for i in range(num_envs):
                output, p_term, i_term, d_term = pids[i].compute(filtered_angles[i].item(), dt)
        ...
        params = {"kp": args.kp, "ki": args.ki, "kd": args.kd, "alpha": args.alpha, "setpoint": args.setpoint}
        ...
        if args.keep_open and not args.headless:
            ...
            try:
                for step in range(args.replay_steps):
                    ...
                    if step % 50 == 0:
                        print(
                            f"Replay step {step}/{args.replay_steps}: "
