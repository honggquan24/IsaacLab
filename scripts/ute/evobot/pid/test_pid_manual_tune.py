...
Usage:
    ...
    ./isaaclab.sh -p scripts/ute/evobot/pid/test_pid_manual_tune.py \
        --num_envs 1
...
def print_balance_metrics(metrics, title: str):
    ...
    if metrics["settling_time"] is not None:
        ...
        print("  ├─ Settling time:        Did not settle within test duration")
...
def main():
    ...
    print(
        f"Target orientation: roll={np.rad2deg(args_cli.target_roll):.1f}°, pitch={np.rad2deg(args_cli.target_pitch):.1f}°"  # noqa: E501
    )
    if args_cli.apply_disturbance:
        print(
            f"Disturbance: {np.rad2deg(args_cli.disturbance_magnitude):.1f}° at t={args_cli.disturbance_time:.1f}s for {args_cli.disturbance_duration:.1f}s"  # noqa: E501
        )
    print("\nPID GAINS:")
    ...
    print(
        f"\n{'Time':>8s} {'RollTgt':>10s} {'RollAct':>10s} {'PitchTgt':>10s} {'PitchAct':>10s} {'RollErr':>10s} {'PitchErr':>10s}"  # noqa: E501
    )
    print("-" * 78)
    ...
    try:
        while simulation_app.is_running() and elapsed_time < args_cli.test_duration:
            with torch.inference_mode():
                ...
                if (
                    args_cli.apply_disturbance
                    and not disturbance_applied
                    and elapsed_time >= args_cli.disturbance_time
                    and elapsed_time < args_cli.disturbance_time + args_cli.disturbance_duration
                ):
            ...
            with torch.inference_mode():
                ...
                if step_count % args_cli.log_interval == 0:
                    ...
                    if step_count % args_cli.print_interval == 0:
                        ...
                        print(
                            f"{elapsed_time:8.2f} {np.rad2deg(roll_target_val):10.2f} {np.rad2deg(roll_actual_val):10.2f} "  # noqa: E501
...
""")
    ...
    print("=" * 80)
    print("./isaaclab.sh -p scripts/ute/evobot/pid/test_pid_manual_tune.py \\")
    print(f"    --num_envs {args_cli.num_envs} \\")
    print("    --kp_roll <new_kp> --ki_roll <new_ki> --kd_roll <new_kd> \\")
    print("    --kp_pitch <new_kp> --ki_pitch <new_ki> --kd_pitch <new_kd>")
    print("=" * 80)
