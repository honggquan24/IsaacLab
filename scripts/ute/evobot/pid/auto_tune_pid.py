...
Usage:
    ./isaaclab.sh -p scripts/ute/evobot/pid/auto_tune_pid.py \
        --epochs 20 --headless
"""
...
def main():
    ...
    from isaaclab_assets.evobot.navigation.velocity.velocity_env_cfg_play import EvobotVelocityBalanceEnvCfg
    ...
    print("✓ Using environment: EvobotVelocityBalanceEnvCfg (Play)")
    print(f"✓ Episode length: {env_cfg.episode_length_s}s")
    print(f"✓ Simulation dt: {env_cfg.sim.dt}s")
    print("✓ Disabled terminations (except timeout)")
    print()
    ...
    print(
        f"{'Epoch':>5} | {'KP':>7} | {'KI':>7} | {'KD':>7} | {'Cost':>10} | {'Survived':>8} | {'Time':>6} | {'Status':>10}"  # noqa: E501
    )
    print("-" * 90)
    ...
    try:
        for epoch in range(args.epochs):
            ...
            cost, survived, survival_time, avg_error, max_error = evaluate_pid(env, kp, ki, kd, args.max_steps, dt)
            ...
            print(
                f"{epoch:5d} | {kp:7.3f} | {ki:7.3f} | {kd:7.3f} | {cost:10.2f} | {survived_str:>8} | {survival_time:6.2f} | {status:>10}"  # noqa: E501
            )
            ...
            if epoch < args.epochs - 1:  # Don't adjust on last epoch
                ...
                if no_improvement_count >= 5:
                    ...
                    print("  ⚠️  Slow progress. Increasing exploration...")
        ...
        with open(csv_file, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["epoch", "kp", "ki", "kd", "cost", "survived", "survival_time", "avg_error", "max_error"])
        ...
        axes[1, 0].axhline(
            y=args.episode_length, color="r", linestyle="--", linewidth=2, label=f"Max Time={args.episode_length}s"
        )
        ...
        with open(config_file, "w") as f:
            f.write("# Best PID Configuration\n")
