...
Usage:
    ...
    ./isaaclab.sh -p scripts/ute/evobot/comparison_tools/compare_pid_rl.py \
...
from isaaclab_assets.evobot.mdp import VelocityPIDActionTermCfg
...
class ControllerComparator:
    ...
    def evaluate_rl(self, checkpoint_path: str | None = None):
        ...
        if checkpoint_path:
            ...
            print("Evaluating Random RL Policy (no checkpoint provided)")
        print(f"{'=' * 60}\n")
        ...
        if checkpoint_path and os.path.exists(checkpoint_path):
            ...
            try:
                from isaaclab_assets.evobot.navigation.velocity.agents import rsl_rl_ppo_cfg
    ...
    def _run_evaluation(self, controller_type: str, action_manager, policy=None):
        ...
        while episode_count < self.num_episodes and step_count < max_steps:
            ...
            processed_action = action_manager.get_term(
                "velocity_pid" if controller_type == "pid" else "wheel_effort"
            ).processed_actions
            ...
            if newly_done_mask.any():
                ...
                for env_id in done_ids:
                    ...
                    self.metrics[controller_type]["action_rate"].append(
                        torch.mean(torch.norm(action_rate[env_id], dim=-1)).item()
                    )
                    self.metrics[controller_type]["action_jerk"].append(
                        torch.mean(torch.norm(action_jerk[env_id], dim=-1)).item()
                    )
                    ...
                    if len(env_actions_history[env_id_int]) > 0:
                        self.metrics[controller_type]["actions_history"].append(
                            np.array(env_actions_history[env_id_int])
                        )
    ...
    def compare_and_save(self, output_dir: str = "comparison_results"):
        ...
        if "success_rate" in results["pid"] and "success_rate" in results["rl"]:
            pid_success = (
                f"{results['pid']['success_rate']['value'] * 100:.1f}% ({results['pid']['success_rate']['count']})"
            )
            rl_success = (
                f"{results['rl']['success_rate']['value'] * 100:.1f}% ({results['rl']['success_rate']['count']})"
            )
