...
Usage:
    ...
    python scripts/ute/evobot/comparison_tools/demo_comparison.py
    ...
    python scripts/ute/evobot/comparison_tools/demo_comparison.py --no-show
"""
...
def plot_comparison(df_rl, df_pid):
    ...
    # PID tracking
    ax1.plot(
        df_pid["time"], df_pid["vx_actual"], "-", color=pid_color, linewidth=2.0, label="PID", alpha=0.85, zorder=3
    )
    ...
    # PID tracking
    ax2.plot(
        df_pid["time"], df_pid["wz_actual"], "-", color=pid_color, linewidth=2.0, label="PID", alpha=0.85, zorder=3
    )
    ...
    print(
        f"  {'RMSE (m/s)':<23} {rl_vx_rmse:>12.6f} {pid_vx_rmse:>12.6f} {(1 - rl_vx_rmse / pid_vx_rmse) * 100:>11.1f}%"
    )
    print(f"  {'MAE (m/s)':<23} {rl_vx_mae:>12.6f} {pid_vx_mae:>12.6f} {(1 - rl_vx_mae / pid_vx_mae) * 100:>11.1f}%")
    print(
        f"  {'Max Error (m/s)':<23} {rl_vx_max:>12.6f} {pid_vx_max:>12.6f} {(1 - rl_vx_max / pid_vx_max) * 100:>11.1f}%"
    )

    print(f"\n{'Angular Velocity (wz):'}")
    print(
        f"  {'RMSE (rad/s)':<23} {rl_wz_rmse:>12.6f} {pid_wz_rmse:>12.6f} {(1 - rl_wz_rmse / pid_wz_rmse) * 100:>11.1f}%"  # noqa: E501
    )
    print(f"  {'MAE (rad/s)':<23} {rl_wz_mae:>12.6f} {pid_wz_mae:>12.6f} {(1 - rl_wz_mae / pid_wz_mae) * 100:>11.1f}%")
    print(
        f"  {'Max Error (rad/s)':<23} {rl_wz_max:>12.6f} {pid_wz_max:>12.6f} {(1 - rl_wz_max / pid_wz_max) * 100:>11.1f}%"  # noqa: E501
    )
