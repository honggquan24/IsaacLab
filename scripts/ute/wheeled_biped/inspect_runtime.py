...
SUS_POSE = torch.tensor([[0.0, 0.0, 1.0, 0.7071, 0.7071, 0.0, 0.0]], device=DEV)
...
def manual_loop(label, setup):
    ...
    for step in range(80):
        ...
        if step % 20 == 0:
            ...
            ks = robot.data.joint_stiffness[0]
            kd = robot.data.joint_damping[0]
            print(
                f"  step {step:3d}  wheelvel R/L = {v[wR].item():+.2f}/{v[wL].item():+.2f}  "
