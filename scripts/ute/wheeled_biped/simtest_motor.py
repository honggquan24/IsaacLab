...
def main():
    ...
    robot_cfg = WHEELED_BIPED_CFG.replace(
        prim_path="/World/Robot",
        init_state=WHEELED_BIPED_CFG.init_state.replace(pos=(0.0, 0.0, spawn_z)),
    )
    ...
    knee_ids = ids("right_knee_joint_1", "right_knee_joint_2", "left_knee_joint_1", "left_knee_joint_2")
    ...
    while simulation_app.is_running():
        ...
        if hip_ids:
            robot.set_joint_position_target(hip_target, joint_ids=torch.tensor(hip_ids, device="cuda:0"))
        if wheel_ids:
            robot.set_joint_velocity_target(wheel_target, joint_ids=torch.tensor(wheel_ids, device="cuda:0"))
        ...
        if step % print_every == 0:
            ...
            print(
                f"{t:6.2f} | {pos[2].item():6.3f} {tilt:6.1f} | "
