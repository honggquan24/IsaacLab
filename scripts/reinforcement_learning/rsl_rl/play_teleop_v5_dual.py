...
Chạy:
    python scripts/reinforcement_learning/rsl_rl/play_teleop_v5_dual.py \
        --task Isaac-Legged-V5-Wheel --num_envs 1 \
...
DEFAULT_FWD  = "logs/rsl_rl/legged_v5_wheel_mimic/2026-06-18_02-19-07/model_3500.pt"
DEFAULT_TURN = "logs/rsl_rl/legged_v5_wheel_mimic/2026-06-18_00-59-33/model_600.pt"

parser = argparse.ArgumentParser(description="Dual-policy teleop V5 wheel — camera + HUD.")
parser.add_argument("--disable_fabric",   action="store_true", default=False)
parser.add_argument("--num_envs",         type=int,   default=1)
parser.add_argument("--task",             type=str,   default="Isaac-Legged-V5-Wheel")
...
parser.add_argument("--switch_thr",       type=float, default=0.02,
                    help="|wz| > thr → dùng TURN policy.")
parser.add_argument("--cam_width",        type=int,   default=640)
parser.add_argument("--cam_height",       type=int,   default=480)
parser.add_argument("--base_link",        type=str,   default="base",
                    help="Tên link gắn camera (xem robot.data.body_names).")
...
from isaaclab.envs import (
    DirectMARLEnv, DirectRLEnvCfg, DirectMARLEnvCfg,
...
def speed_color(val, mx):
    ...
    return lerp_c(C_GREEN, C_ORANGE, t / 0.6) if t < 0.6 \
        else lerp_c(C_ORANGE, C_RED, (t - 0.6) / 0.4)
...
class HUDPanel:
    ...
    def __init__(self, cam_w, cam_h, vy_max, wz_max):
        ...
        pygame.display.set_caption("V5 Teleop — Dual Policy")
    ...
    def _make_nocam(self):
        ...
        s.blit(t, (self.CW // 2 - t.get_width() // 2,
                   self.CH // 2 - t.get_height() // 2))
    ...
    def poll(self):
        ...
        for ev in pygame.event.get():
            if ev.type == pygame.QUIT:
                ...
                if ev.key == pygame.K_ESCAPE: quit_f = True
                if ev.key == pygame.K_r:      reset   = True
        k = pygame.key.get_pressed()
        vy = (self.vy_max if k[pygame.K_w] else 0.) \
           - (self.vy_max if k[pygame.K_s] else 0.)
        wz = (self.wz_max if k[pygame.K_a] else 0.) \
           - (self.wz_max if k[pygame.K_d] else 0.)
        if k[pygame.K_SPACE]: vy = wz = 0.
    ...
    def _row(self, x, y, label, val_str, color=None, unit=""):
        ...
        self.screen.blit(self.fT.render(unit, True, C_DIM),
                         (x + 170 + vs.get_width() + 4, y + 4))
        return y + 32
    ...
    def draw(self, frame, vy, wz, name, is_turn, dbg, fps):
        ...
        if dbg:
            ...
            if abs(dbg["roll"]) > 35:
                self.screen.blit(
                    self.fL.render("⚠  NGUY CƠ LẬT NGANG", True, C_RED), (hx, y+4))
        else:
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg,
    ...
    env_cfg.scene.terrain.usd_path        = (
    ...
    panel       = HUDPanel(args_cli.cam_width, args_cli.cam_height,
                           args_cli.vy_max, args_cli.wz_max)
    ...
    try:
        while simulation_app.is_running():
            ...
            policy, active_name = (
                (policy_turn, "TURN  model_600")
                if is_turn else
                (policy_fwd,  "FWD   model_3500")
            )
